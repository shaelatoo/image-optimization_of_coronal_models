function harmonic_trypoint,simplex,y,psum,windex,fac, $
  ...
  ;;                                                            ;;
  ;; Modified: 11/30/15 - modified to use new version,          ;;
  ;;     find_angle_values2 and to accept variable spcCoords for;;
  ;;     use by same (sij)                                      ;;
  ;;           04/19/16 - added diff_power keyword and          ;;
  ;;     implementation                                         ;;
  ;;           02/28/17 - removed input variable maxlvar and put;;
  ;;     in pfss_opt_parameters                                 ;;
  ;;           05/31/17 - added rindex keyword and passed on to ;;
  ;;     pfss_potl_field routine, set rgrid=3                   ;;
  ;;                                                            ;;
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if KEYWORD_SET(bounds) then begin
    ...
    if MIN(updiff) lt 0 or MIN(lowdiff) lt 0 then penalty=pen_mult*penalty
  endif
  ...
  if penalize_netflux ne 0. then begin
    mag=INV_SPHERICAL_TRANSFORM(newmagt,cth,lmax=lmax)
    penalty=penalty+penalize_netflux*TOTAL(mag)^2.
    if penalize_magchange ne 0. then penalty=penalty+ $
        penalize_magchange*TOTAL(magweights*ABS(mag-mag0))
  endif else if penalize_magchange ne 0. then begin
    mag=INV_SPHERICAL_TRANSFORM(newmagt,cth)
    penalty=penalty+penalize_magchange*TOTAL(magweights* $
        ABS(mag-INV_SPHERICAL_TRANSFORM(magt,cth)))
  endif
