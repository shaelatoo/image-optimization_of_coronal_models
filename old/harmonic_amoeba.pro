function harmonic_amoeba,magfile,angles,coords,ftol,scale, $
 ...
 ; check that pfss_opt_parameters have been defined
  ...
  if n_elements(rgrid) eq 0 then rgrid=rgrid_default
...
; note: this line is no longer needed due to changes in 
;   pfss_mag_create_sj implemented after this routine became 
;   obsolete; was commented out in 12/2017 to facilitate creation
;   of comparison figures for harmonic_amoeba vs harmonic_amoeba2
;   for presentation at the fall agu meeting 
  ...
  if KEYWORD_SET(inittrans) then begin
    ...
    magt=SPHERICAL_TRANSFORM(magnetogram,cth,lmax=nlat0)
  endelse
  ...
  psum=0
  y=FLTARR(nvert+1)
  for i=0,nvert do y[i]=HARMONIC_TRYPOINT(simplex,y,psum,i,1., $
    angles,coords,spcCoords,/penalty_only,bounds=bounds, $
    weights=weights,magweights=magweights,penalize_magchange= $
    penalize_magchange,rindex=rindex,_extra=extraKeys)
...
;      print,'contract'
      ysave = y[highest]
      ytry=HARMONIC_TRYPOINT(simplex,y,psum,highest,contraction, $
        angles,coords,spcCoords,bounds=bounds,weights= $
    ...
    if (cnt mod 100) eq 0 then begin
      ...
      if KEYWORD_SET(magmovset) then begin
        ...
        newmag=INV_SPHERICAL_TRANSFORM(bestmagt,cth)
